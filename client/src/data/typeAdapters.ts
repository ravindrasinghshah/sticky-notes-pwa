...
export const convertFirebaseBucketToBucket = (
...
export const convertFirebaseBucketWithCountToBucketWithCount = (
  firebaseBucket: FirebaseBucketWithCount,
): BucketWithCount => ({
  ...convertFirebaseBucketToBucket(firebaseBucket),
  noteCount: firebaseBucket.noteCount,
});

export const convertFirebaseNoteWithBucketsToNoteWithBuckets = (
  firebaseNote: FirebaseNoteWithBuckets,
): NoteWithBuckets => {
  const note = convertFirebaseNoteToNote(firebaseNote);
  return {
    ...note,
    sharedBuckets: firebaseNote.sharedBuckets.map(
...
export const convertBucketToFirebaseBucket = (
  bucket: Omit<Bucket, "id" | "userId" | "createdAt" | "updatedAt">,
): Omit<FirebaseBucket, "id" | "userId" | "createdAt" | "updatedAt"> => ({
  name: bucket.name,
...
export const convertNoteToFirebaseNote = (
  note: Omit<Note, "id" | "userId" | "createdAt" | "updatedAt">,
): Omit<
  FirebaseNote,
  "id" | "userId" | "createdAt" | "updatedAt" | "sharedBucketIds"
> => ({
  title: note.title,
...
export const convertInsertBucketToFirebaseBucket = (
  bucket: import("./schema").InsertBucket,
): Omit<FirebaseBucket, "id" | "userId" | "createdAt" | "updatedAt"> => ({
  name: bucket.name,
  description: bucket.description || undefined,
  color: bucket.color || "primary",
  icon: bucket.icon || "sticky-note",
  status: bucket.status === "archived" ? "archived" : null,
});

export const convertInsertNoteToFirebaseNote = (
  note: import("./schema").InsertNote,
): Omit<
  FirebaseNote,
  "id" | "userId" | "createdAt" | "updatedAt" | "sharedBucketIds"
> => ({
